...
DBUSER="${CONFIG_omero_db_user:-omero_database_user}"
DBNAME="${CONFIG_omero_db_name:-postgres_database}"
...
while ! psql -h "$DBHOST" -p "$DBPORT" -U "$DBUSER" "$DBNAME" >/dev/null 2>&1 < /dev/null; do
    i=$(($i+1))
    if [ $i -ge 50 ]; then
        echo "$(/bin/date) - postgres:5432 still not reachable, giving up"
        exit 1
    fi
    echo "$(/bin/date) - waiting for postgres:5432..."
    /usr/bin/sleep 1
done
